...
//---------Create Deleteparams Api-------//

const deletebyparams = async function (req, res) {
    try {
        const blogId = req.params.blogId
        //let authorId = req.params.NewUser
        //console.log (authorId)
        const checkBlog=await newblog.findOne({_id:blogId});
        if (checkBlog.isDeleted == true) {
            return res.status(400).send({ status: false, msg: "block does not exist" })
        }
       // const checkBlog=await newblog.findOne({})
        await newblog.findOneAndUpdate({ blogId }, { $set: { isDeleted: true, deletedAt: new Date() } })
...
//--------Create DelBySpecificfield Api---------//

const delBySpecificField = async function (req, res) {
    try {
        let data = req.query.blogId
